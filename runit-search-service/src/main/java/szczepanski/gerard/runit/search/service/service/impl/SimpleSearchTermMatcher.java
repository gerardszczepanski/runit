 ...
 * It checks, if splitted by space searchResutlName starts with searchTerm, both in lower case.
 * If yes, then matching is positive. 
 * 
 * Positive matching: 
 * 
 * searchTerm - google
 * positive: google chrome
 * positive: chrome google
 * 
...
public class SimpleSearchTermMatcher implements SearchTermMatcher {
	...
	private boolean processSplitedSearchResultName(String searchTerm, String[] searchResultNameSplitted) {
		for (int i = 0; i < searchResultNameSplitted.length; i++) {
			if (isSplittedResultNameStartsWithSearchTerm(searchTerm, searchResultNameSplitted[i])) {
				return true;
			}
		}
	...
	private boolean isSplittedResultNameStartsWithSearchTerm(String searchTerm, String spliiedSearchResultNamePart) {
		return spliiedSearchResultNamePart.toLowerCase().startsWith(searchTerm.toLowerCase());
	}
